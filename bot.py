...
url = deviantart_twitch_api.build_search_url(game)
print(url)
...
while json_search_result['has_more'] == True and len(all_search_results) <= 250:
	...
	json_search_result = deviantart_twitch_api.get_result(extended_url)
	for obj in json_search_result['results']:
...
while True:
	...
	time.sleep(300) # every 5 minutes, choose a random (popular) piece of art to post in chat
